                ...
                <Column class="" header="สถานะอนุมัติ" style="width: 10%">
                    <template #body="{ data }">
                        <div v-if="data.approve.slice(-1)[0].statusapprove === 'รออนุมัติ'">
                            <div class="font-bold" style="color: #ff7315" >{{ data.approve.slice(-1)[0].statusapprove }}</div> 
                        </div>
                        <div v-if="data.approve.slice(-1)[0].statusapprove === 'อนุมัติ'">
                            <div class="text-green-500 font-bold" >{{ data.approve.slice(-1)[0].statusapprove }}</div> 
                        </div>
                        <div v-if="data.approve.slice(-1)[0].statusapprove === 'ไม่อนุมัติ'">
                            <div class="text-red-500 font-bold" >{{ data.approve.slice(-1)[0].statusapprove }}</div> 
                        </div>
                ...
                <Column class="" header="เพิ่มเติม" style="width: 10%">
                    <template #body="{ data }">
                        <!-- ให้แสดงค่า statusapprove ของแต่ละ Item ใน Column -->
                         <div v-if="data.approve.slice(-1)[0].statusapprove === 'รออนุมัติ'">
                            <!-- กรณีรอการอนุมัติ -->
                            <Button @click="approvepartner(data._id)" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mx-2"
                            style="background-color: #C21010">อนุมัติ</Button>
                            <Button @click="unapprovepartner(data._id)" class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded"
                            style="background-color: #C21010">ไม่อนุมัติ</Button>
                        </div>
                    </template>
                </Column>
...
export default {
    ...
    setup() {
        ...
        const approvepartner = async (_id) => {
            try{
                const response = await axios.put(`${process.env.VUE_APP_API}partner/approve/${_id}`, {},{
            headers: {
              token: localStorage.getItem("token"),
            },
          });
